...
public class EditPriceListDialog extends JDialog {
	...
	public EditPriceListDialog(PriceList priceList, PriceListsModel priceListModel) {
		PriceListManager priceListManager = ManagerFactory.getInstance().getPriceListManager();
		this.priceListsModel = priceListModel;
		...
		{
			startField = new JTextField(priceList.getStartDate().toString());
		...
		{
			endField = new JTextField(priceList.getExpirationDate().toString());
		...
		{
			singleField = new JTextField(priceList.getPrices().get("SINGLE").toString());
		...
		{
			double1_1Field = new JTextField(priceList.getPrices().get("DOUBLE1_1").toString());
		...
		{
			double2Field = new JTextField(priceList.getPrices().get("DOUBLE2").toString());
		...
		{
			triple1_1_1Field = new JTextField(priceList.getPrices().get("TRIPLE1_1_1").toString());
		...
		{
			triple1_2Field = new JTextField(priceList.getPrices().get("TRIPLE1_2").toString());
		...
		for (String service : services) {
		    ...
		    Double servicePrice = priceList.getPrices().get(service);
		    serviceField.setText(servicePrice != null ? servicePrice.toString() : "");
		...
		}
		{
			...
			getContentPane().add(buttonPane, BorderLayout.SOUTH);
			{
				...
				okButton.addActionListener(e ->{
					...
					priceList.setExpirationDate(endDate);
					priceList.setPrices(priceMap);
